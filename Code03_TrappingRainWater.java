...
public class Code03_TrappingRainWater {
    ...
    public static int trapRainWater(int[][] heightMap) {
        ...
        for (int col = 0; col < M - 1; col++) {
            isEnter[0][col] = true;
            heap.add(new Node(heightMap[0][col], 0, col)); //上
        }
        ...
        for (int col = M - 1; col > 0; col--) {
            ...
            heap.add(new Node(heightMap[N - 1][col], N - 1, col)); // 左
        }
        for (int row = N - 1; row > 0; row--) {
            isEnter[row][0] = true;
            heap.add(new Node(heightMap[row][0], row, 0)); // 右
        }
        int water = 0;
        int max = 0;
        while (!heap.isEmpty()) {
            ...
            if (r > 0 && !isEnter[r - 1][c]) {
                ...
                isEnter[r - 1][c] = true;
                heap.add(new Node(heightMap[r - 1][c], r - 1, c)); //入栈
            }
